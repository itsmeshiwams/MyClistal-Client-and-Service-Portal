...
export default function RecentActivityRow({ activity }) {
  const actionInfo = ACTION_MAP[activity.action] || { label: activity.action, icon: FileText };
  const Icon = actionInfo.icon;
  const timeLabel = new Date(activity.createdAt).toLocaleString();

  return (
    <li className="flex items-center gap-3  border-b cursor-pointer hover:bg-gray-50  last:border-b-0 p-2 transition">
      ...
      <div className="flex-1">
        <p className="text-gray-800 text-base font-medium">{actionInfo.label}</p>
